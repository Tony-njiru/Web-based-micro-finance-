...
if(!$Function->isLogin()){
	...
	if($prof['right'] > 0){
		...
		if(isset($_POST['delu'])){
			...
			if(isset($_POST['unswitch'])){
			if($flash->query("UPDATE `user` SET `switched`='0' WHERE `username`='{$_POST['duser']}' LIMIT 1")){
				echo '<p><div class="success">Successfully User Recovered! </div></p>';
			}
		}
		if(isset($_GET['q'])){
			$searchr = $_GET['q'];
			$suser = $flash->prepare("SELECT * FROM `user` WHERE MATCH(username) AGAINST(:sch) AND `switched`>0 
			...
			if($suser->rowCount() < 1){
				echo '<div class="info">User Not Found or Not Switched!</div>';
			}
			...
			Member Since: '.$sresult['joined'].'</b></div><div class="left">
			<form method="POST">
			<input type="hidden" name="duser" value="'.$sresult['username'].'"/>
			<button style="float:left;" name="delu"> Delete</button> <br/><br/>
			<button name="unswitch"> Restore</button></form></div>
			<hr/>';		
			}
			echo '</div></p>';
			$statement = "`user` WHERE MATCH(username) AGAINST('{$searchr}') AND `switched`>0";
...
//Rest User
            $hssuser = $flash->query("SELECT * FROM `user` WHERE id>0 AND `switched`>0");
			$ssuser = $flash->query("SELECT * FROM `user` WHERE id>0 AND `switched`>0 ORDER BY `id` ASC
			LIMIT {$startpoint} , {$limit}");
			echo '<p><div class="main"><div class="main title">All Switched Users</div> ';
			if($ssuser->rowCount() > 0){
			...
			Member Since: '.$ssresult['joined'].'</b></div><div class="left">
			<form method="POST">
			...
			<hr/>';	
				...
				echo '<div class="info">No Switched User Yet!</div>';
			}
			echo '</div></p>';
			$statement = "`user` WHERE id>0 AND `switched`>0";
            $query = $flash->query("SELECT * FROM {$statement} ORDER BY id DESC LIMIT {$startpoint} , {$limit}");
